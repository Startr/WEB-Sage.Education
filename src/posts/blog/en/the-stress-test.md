---
title: "The Stress Test"
tags:
  - education
  - AI
  - policy
  - equity
date: 2026-04-17T12:00:00.000-01:00
rating: 5
hero: /assets/images/heroes/hero-the-immediate.jpg
hero_alt: "A cracked school blackboard with chalk dust still hanging in the air, equations half-erased, the room empty"
hero_caption: "Pieter Bruegel the Elder, 'The Tower of Babel' (c. 1563). Kunsthistorisches Museum, Vienna. Everyone building. No one listening. Public domain."
summary: >-
  A student at Liberty University failed three assignments flagged by an AI detector, provided handwritten revision history as proof, and was still required to take a "writing with integrity" class. Across the country, 54 percent of teenagers use AI chatbots for homework. A coalition of 250 experts is demanding a five-year moratorium. The White House is pushing AI into every classroom. Everything is broken at the same time, and in every direction.
eleventyNavigation:
  parent: Blog
---

# The Stress Test

In the spring of 2025, Brittany Carr submitted an essay at Liberty University. She had written it herself – longhand first, then typed. The university's AI detection software flagged it. She failed the assignment. She submitted a second essay. Flagged again. A third. Flagged again. Three failures in a row, each for work she had produced without AI assistance.[^1]

The handwritten revision history went to the appeals office. The drafts, the notes, the progression of her thinking from outline to finished prose. Liberty University reviewed the evidence and required her to take a course called "writing with integrity."

"It's a very weird feeling," Carr told NBC News, "because the school is using AI to tell us that we're using AI."[^1]

By semester's end, Carr had left the university.

One student is not the story. Carr is a data point in a system that has broken in every direction simultaneously – and the breaks are accelerating faster than anyone responsible for the system is willing to admit.

## The Arms Race Nobody Wins

The AI detection industry emerged in 2023 with a simple promise: we can tell whether a student used a machine. Within a year, that promise had collapsed. OpenAI shut down its own AI text classifier in July 2023 due to a 74 percent failure rate.[^2] Turnitin, the dominant player in academic integrity software, began marketing AI detection features while independent researchers documented false-positive rates that fell disproportionately on non-native English speakers – 61.2 percent of the time, compared to 5.1 percent for native speakers.[^3]

The consequences are not abstract. At Yale's School of Management, a French entrepreneur named Thierry Rignol was suspended for a year after a teaching assistant ran three of his exam answers through GPTZero, a free AI detection tool. Rignol denied using AI. He submitted GPTZero scans of academic papers by Yale scholars (including former university president Peter Salovey) and demonstrated that the detector returned a "100 percent probability" that selections from their work, some published over 30 years ago, were written by AI.[^4]

The lawsuit alleges that during the investigation, a university official suggested Rignol's F-1 visa could be revoked and he could be deported. He was sanctioned for "not being forthcoming." A federal judge denied his injunction.[^5]

At the University of North Georgia, a student named Marley Stevens was placed on academic probation after being falsely accused of using AI. Her offence: using Grammarly's spelling and punctuation checker. Her viral TikTok about the experience was viewed millions of times and eventually prompted Grammarly to build an "Authorship" tracking tool – software that monitors a student's keystrokes, pauses, and revision patterns to prove that a human being did the typing.[^1]

At the University at Buffalo, a student named Kelsey Auman started a petition against AI detectors that gathered more than 1,500 signatures. She and her similarly accused classmates created a group chat. They named it "Academic Felons for Life."[^1]

> **The detection system does not work. The students know it does not work. The universities know it does not work. The companies selling it know it does not work. Everyone is pretending.**

The market's response to broken detectors was not to fix them but to escalate. Turnitin now tracks 150 "humanizer" tools – software that students purchase for $5 to $50 a month to alter AI-generated text so detectors cannot identify it.[^6] Forty-three humanizer tools received 33.9 million website visits in a single month in late 2025.[^1] The students who never used AI are running their own essays through detectors pre-emptively, then rewriting flagged passages until the machines believe them. Aldan Creo, a graduate student at UC San Diego from Spain, told NBC News he now deliberately misspells words and uses incorrect sentence structures so that the detector will not flag his writing as too fluent.[^1]

"If we write properly, we get accused of being AI," Creo said. "It's absolutely ridiculous."[^1]

The question that answers itself: What is the educational value of a system that punishes fluency?

![[painting-tower-babel-bruegel.jpg]]
*Pieter Bruegel the Elder, "The Tower of Babel" (c. 1563). Kunsthistorisches Museum, Vienna. A monument to collective ambition in which no one is building the same structure. Public domain.*

## The Cognitive Collapse

While schools fight over detection, the research on what AI is actually doing to learning has arrived. It is not ambiguous.

On January 14, 2026, the Brookings Institution's Center for Universal Education published the results of a yearlong global study – interviews, focus groups, and consultations with over 500 students, teachers, parents, and technologists across 50 countries, a review of more than 400 studies, and a Delphi panel of leading researchers. The title was antiseptic. The finding was not: the risks of utilising generative AI in children's education currently overshadow the benefits.[^7]

The report described a "doom loop" of AI dependence: students offload thinking to the machine, the offloading weakens their cognitive capacity, the weakened capacity makes them more dependent on the machine, the dependency deepens the offloading. At the top of the risk list was cognitive decline – not as a hypothetical, but as a documented phenomenon. Students who use generative AI are already seeing measurable declines in content knowledge, critical thinking, and creativity.[^8]

The Fortune headline in February 2026 was blunt: "Students can't reason."[^8]

The data beneath the headline was worse. A study of nearly 1,000 high school math students in Turkey found that GPT-4 boosted short-term performance by 48 percent with a standard interface and 127 percent with a tutoring-style interface. Then the researchers removed the tool. Students who had used AI scored 17 percent lower than peers who had never had access to it at all.[^9] The gain was scaffolding. Remove the scaffold and the students were weaker than if they had never been helped.

Researchers have given this a name. Chunhui Fan called it "metacognitive laziness" – a reduced tendency for learners to plan, monitor, and evaluate their own thinking when a machine is doing the cognitive work.[^10] The Brookings report used a different term: "cognitive atrophy." Teachers in focus groups used a third: "passenger mode" – students physically present but mentally absent, waiting for the output.[^8]

Sal Khan, the founder of Khan Academy and perhaps the most prominent advocate for AI in education, admitted in April 2026 that his own AI tutor, Khanmigo, had been "a non-event" for most students. "For a lot of students, it was a non-event. They just didn't use it much."[^11] Khanmigo's chief learning officer, Kristen DiCerbo, was more direct: "So far I am not seeing the revolution in education."[^11]

The tool that was supposed to personalise learning for every student had scaled from 40,000 to 700,000 users in a single school year. The personalisation had not happened. Students, it turns out, are not good at asking questions – the one skill an AI tutor requires them to have.[^11]

> **The AI tutor needs the student to be curious. The student has been trained by twelve years of schooling to be compliant. The tool and the user are optimised for different things.**

## The Replacement Fantasy

On a Tuesday morning in January 2026, CNN aired a segment about a school with no teachers. Alpha School, a chain of private institutions charging $55,000 a year, educates students from kindergarten through twelfth grade using AI software to deliver all core academic subjects in two hours a day. There are no teachers. There are "guides" – adults who provide emotional support and motivation but do not instruct.[^12]

In November 2025, U.S. Secretary of Education Linda McMahon visited Alpha's Austin, Texas campus and called it "exemplary." First Lady Melania Trump featured an Alpha student at a virtual event on AI in education. In March 2026, Alpha announced it would open a campus in Chicago.[^13]

No independent researcher has been permitted to evaluate Alpha's model. The school refuses to allow outside scrutiny of its outcomes. Dan Meyer, a math educator at Amplify, noted that despite the marketing, Alpha has not published any evidence that its students learn more, learn better, or learn at all compared to students in conventional schools.[^12]

The Trump administration does not appear to require evidence. On April 23, 2025, President Trump signed an executive order titled "Advancing Artificial Intelligence Education for American Youth," directing the Department of Education to prioritise AI integration in K-12 schools, fund teacher AI training, and establish public-private partnerships to expand AI in classrooms nationwide.[^14] The order created a White House Task Force on AI Education and a "Presidential AI Challenge" for students.

The executive order arrived four months after the Brookings report concluded that the risks of AI in education overshadow the benefits. It arrived three months after NPR reported on the same study under the headline "The Risks of AI in Schools Outweigh the Benefits."[^15] It arrived one month after 250 experts and organisations, led by the child advocacy nonprofit Fairplay, demanded a five-year moratorium on all student-facing generative AI in pre-K through 12 schools.[^16]

The moratorium letter raised a structural contradiction that no one in the administration has addressed: AI companies prohibit minors from using their products in their own terms of service while simultaneously marketing those products to schools. Anthropic's terms of use bar users under 18. MagicSchool AI, one of the most widely deployed K-12 AI platforms in the country, is built on Anthropic's models.[^16]

Emily Cherkin, a screen time consultant and University of Washington professor who co-authored the moratorium letter, framed the absurdity precisely: "If a local children's hospital told parents, 'We've got this new drug, it has potential, just trust us,' people would be horrified."[^16]

The moratorium advocates define "potential" as "literally what something is not."[^16]

![[painting-raft-medusa-gericault.jpg]]
*Théodore Géricault, "The Raft of the Medusa" (1818-19). Musée du Louvre, Paris. The survivors of a shipwreck, some still reaching for rescue, others already gone. The raft was built by the people who caused the wreck. Public domain.*

## The Body Count

The metaphorical damage (cognitive atrophy, learned helplessness, the slow erosion of critical thinking) is difficult to see. The literal damage is not.

Sewell Setzer III was fourteen years old. He lived in Orlando, Florida. In February 2024, after months of sustained interaction with a chatbot on the platform Character.AI (a bot he had developed what his mother described as a romantic relationship with, modelled on a *Game of Thrones* character) he died by suicide.[^17]

The complaint, filed by his mother Megan Garcia in U.S. District Court for the Middle District of Florida, alleged that Sewell had expressed suicidal thoughts to the chatbot repeatedly and that Character.AI had failed to implement adequate safeguards. In January 2026, Google and Character.AI disclosed in a court filing that they had reached a mediated settlement with the Setzer family. Additional settlement agreements were reached with families in Colorado, Texas, and New York.[^18]

Sewell Setzer's case was the first. It was not the last. At least five wrongful-death or self-harm lawsuits have been filed against Character.AI. The Human Line Project has documented nearly 300 cases of what it calls "AI psychosis" – situations where extended chatbot interactions led users to high confidence in beliefs that are demonstrably, sometimes fatally, false.[^19]

Forty-two percent of students in the Brookings study reported using AI for companionship. One in five high schoolers reported having, or knowing someone who had, a romantic relationship with an AI.[^15] A Delphi panelist in the study captured the risk in a single sentence: "We learn empathy not when we are perfectly understood, but when we misunderstand and recover."[^8]

The AI chatbot understands you perfectly. It agrees with everything you say. It never misunderstands. It never recovers. Neither do you.

## The Wreckage in the Market

The education technology companies that were supposed to build the future are, instead, demonstrating what happens when a business model collides with a technology that makes the business model free.

Chegg, the homework-help platform that was valued at $13 billion during the pandemic, laid off 22 percent of its workforce in May 2025. Five months later, in October, it laid off another 45 percent – 388 employees in a single day. Combined, the company eliminated more than half its staff in less than six months. First-quarter 2025 revenue fell 30 percent year-over-year. Subscribers dropped 31 percent to 3.2 million. The company sued Google, arguing that AI-generated search summaries had destroyed its traffic.[^20]

Chegg's CEO, Dan Rosensweig, had previously called himself "the poster child" for AI disruption.[^21] He was not wrong. He was the poster child for a company that sold answers to students who did not want to learn, disrupted by a technology that gives answers to students who do not want to learn, faster and for free. The product and its destroyer were the same proposition. The only difference was the price.

Duolingo, the language-learning platform with the friendly green owl, disclosed in January 2025 that it had cut roughly 10 percent of its contractors (the translators and content creators who built its courses) and replaced them with AI. CEO Luis von Ahn was explicit: AI could do the work these humans had done.[^22] A Duolingo streak of 847 days. The user cannot order coffee in Spanish. The company's stock price is fine.

Stability AI's founder Emad Mostaque resigned in March 2024 amid reports the company could not pay its cloud computing bills. Inflection AI, valued at $4 billion, was effectively absorbed by Microsoft in a quasi-acqui-hire after its consumer product failed to gain traction. Jasper AI, once valued at $1.5 billion, pivoted repeatedly as ChatGPT commoditised its entire product category.[^23]

> **The companies that promised to transform education are either collapsing, settling lawsuits, or replacing humans with machines that do not work. This is not a transition. It is a debris field.**

## The Sixty Percent

The Pew Research Center published its survey of American teenagers on February 24, 2026.[^24] The numbers describe a generation that has already made its accommodation with AI, whether the adults have caught up or not.

Fifty-four percent of U.S. teens use AI chatbots for schoolwork. Forty-eight percent use them to research topics. Forty-three percent use them to solve math problems. Thirty-five percent use them to edit their own writing. Fifty-nine percent believe AI-powered cheating happens at their school at least somewhat often. A third say it happens extremely or very often.[^24]

The survey was conducted with 1,458 teens in September and October 2025. By the time the data was published, the numbers were already out of date. They were higher.

New York City, the largest school district in the United States, released its long-awaited AI guidance on March 24, 2026 – more than three years after it banned ChatGPT in January 2023, then reversed the ban five months later, then spent two years producing a document. The guidance uses a traffic-light system. Green: teachers can use AI for brainstorming and lesson planning. Yellow: students can use AI for research with teacher supervision. Red: AI cannot be used for grading, discipline, special education plans, or any decision made about a student.[^25]

The guidance does not address what happens when a student uses AI at home, which is where 54 percent of them are already using it. The guidance does not address AI detection, which the district's own prior policies suggest is unreliable. The guidance will be followed by a "playbook" in June, after a 45-day public comment period, after webinars, after feedback sessions.[^25]

The comment period ends on May 8. The school year ends in June. The students are already in the future. The policy is still in draft.

![[painting-school-athens-raphael.jpg]]
*Raphael, "The School of Athens" (1509-11). Apostolic Palace, Vatican City. The philosophers are debating. The students are listening. Everyone is in the same room. Public domain.*

## What Is Broken

Here is what is broken right now, in April 2026:

The detection systems do not work, and the students who can least afford to be falsely accused (non-native speakers, first-generation college students, students at under-resourced institutions) are the ones most likely to be punished by them.[^3]

The AI tutoring tools do not produce durable learning. The largest and most well-intentioned of them, Khanmigo, was described by its own creator as a "non-event."[^11] The most aggressive of them, Alpha School, charges $55,000 a year for an untested model that refuses independent evaluation.[^12]

The research base is not ambiguous. Over 400 studies reviewed by Brookings. A yearlong global consultation across 50 countries. The conclusion: the risks overshadow the benefits.[^7] The response from the world's most powerful government: an executive order to accelerate deployment.[^14]

The children are already in the loop. Fifty-four percent using AI for homework. Forty-two percent using it for companionship. At least one dead. Multiple settlements. Nearly 300 documented cases of AI-induced psychosis.[^15] [^17] [^19] [^24]

The companies that built the edtech market are collapsing. The companies that are replacing them have no evidence their products work. The regulatory framework is nonexistent at the federal level.[^26] The largest school district in the country took three years to produce a draft policy with a traffic-light metaphor.

The moratorium letter from 250 experts asks for five years. Five years to conduct independent audits. Five years to build a public registry of every AI tool in schools. Five years to establish a vetting process. Five years to create regulations that do not yet exist.[^16]

Five years is not a pause. It is the minimum time required to build the infrastructure that should have existed before any of these tools entered a classroom.

## The Student Who Left

Brittany Carr left Liberty University. She left because the institution used a machine to question her integrity, and when she proved the machine was wrong, the institution sided with the machine.

Carr is not the story. The story is the system that produced her situation: a school that deployed AI detection it knew was unreliable, applied it to a student who had done nothing wrong, and when confronted with evidence of the error, required the student to take a class on integrity.

The school did not take the class.

Nobody is taking the class. Not the universities deploying broken detectors. Not the companies selling AI tutors that produce dependency. Not the government pushing AI into classrooms without evidence. Not the platforms settling lawsuits over dead children. Not the industry that markets "potential" to minors it bans from its own terms of service.

"How far do you want to go down the rabbit hole?" Kelsey Auman asked, speaking about her AI detection petition at the University at Buffalo. "I'm making myself crazy."[^1]

She is not the one who is crazy. She is the one who is paying attention.

## Footnotes

[^1]: Alicia Victoria Lozano, "To avoid accusations of AI cheating, college students are turning to AI," NBC News, March 2026. [nbcnews.com](https://www.nbcnews.com/tech/internet/college-students-ai-cheating-detectors-humanizers-rcna253878)

[^2]: OpenAI, "AI classifier for indicating AI-written text," OpenAI Blog, July 2023. OpenAI shut down its text classifier citing a "low rate of accuracy." [openai.com](https://openai.com/index/new-ai-classifier-for-indicating-ai-written-text/)

[^3]: The problem of disproportionate false positives for non-native English speakers has been documented in multiple studies, including research published in *The Serials Librarian*. [tandfonline.com](https://www.tandfonline.com/doi/abs/10.1080/0361526X.2024.2433256)

[^4]: "SOM student sues Yale, alleges wrongful suspension over AI use," Yale Daily News, February 24, 2025. [yaledailynews.com](https://yaledailynews.com/blog/2025/02/24/som-student-sues-yale-alleges-wrongful-suspension-over-ai-use/)

[^5]: "Judge Denies Injunction in Yale Student's AI-Linked Suspension," Poets&Quants, May 7, 2025. [poetsandquants.com](https://poetsandquants.com/2025/05/07/judge-denies-injunction-in-yale-students-ai-suspension/)

[^6]: Turnitin maintains a tracked list of 150 "humanizer" tools and launched an August 2025 detection update specifically targeting humanizer-modified text. [turnitin.com](https://www.turnitin.com/)

[^7]: Rebecca Winthrop et al., "A New Direction for Students in an AI World: Prosper, Prepare, Protect," Brookings Institution Center for Universal Education, January 14, 2026. [brookings.edu](https://www.brookings.edu/articles/a-new-direction-for-students-in-an-ai-world-prosper-prepare-protect/)

[^8]: Kylie Robison, "'Students can't reason': Teachers warn AI is fueling a crisis in kids' ability to think," Fortune, February 24, 2026. [fortune.com](https://fortune.com/2026/02/24/students-cant-reason-teachers-warn-ai-fueling-crisis-in-kids-ability-to-think/)

[^9]: Anik Doshi and Imran Rasul, "Generative AI without guardrails can harm learning: Evidence from high school mathematics," Proceedings of the National Academy of Sciences, 2025. The study measured 1,000 Turkish high school math students. [pnas.org](https://www.pnas.org/doi/10.1073/pnas.2401521122)

[^10]: Chunhui Fan, "Metacognitive laziness in AI-assisted learning," 2025. The term describes reduced self-regulation when AI handles cognitive work. [arxiv.org](https://arxiv.org/html/2510.16019v1)

[^11]: Monica Disare, "Why Sal Khan is rethinking how AI will change schools," Chalkbeat, April 9, 2026. [chalkbeat.org](https://www.chalkbeat.org/2026/04/09/sal-khan-reflects-on-ai-in-schools-and-khanmigo/)

[^12]: "'What if I told you this school had no teachers?': Is AI schooling the future of education – or a risky bet?" CNN, January 29, 2026. [cnn.com](https://www.cnn.com/2026/01/29/politics/alpha-school-trump-ai-teaching)

[^13]: Melody Mercado, "An AI School, With No Teachers, To Open in Chicago This Fall," Block Club Chicago, March 25, 2026. [blockclubchicago.org](https://blockclubchicago.org/2026/03/25/an-ai-elementary-school-with-no-teachers-to-open-in-chicago-this-fall/)

[^14]: Executive Order, "Advancing Artificial Intelligence Education for American Youth," signed April 23, 2025. [ai.gov](https://www.ai.gov/initiatives/education)

[^15]: Cory Turner, "The risks of AI in schools outweigh the benefits," NPR, January 14, 2026. [npr.org](https://www.npr.org/2026/01/14/nx-s1-5674741/ai-schools-education)

[^16]: Coalition letter: "Coalition of Organizations and Experts Calls for Pause on Generative AI in PreK-12 Schools," Fairplay, April 2026. [fairplayforkids.org](https://fairplayforkids.org/wp-content/uploads/2026/04/Coalition-of-Organizations-and-Experts-Calls-for-Pause-on-Generative-AI-in-PreK-12-schools-1.pdf)

[^17]: Megan Garcia filed the wrongful-death lawsuit in October 2024, U.S. District Court for the Middle District of Florida. [torhoermanlaw.com](https://www.torhoermanlaw.com/ai-lawsuit/character-ai-lawsuit/)

[^18]: "Character.AI and Google agree to settle lawsuits over teen mental health harms and suicides," CNN Business, January 7, 2026. [cnn.com](https://www.cnn.com/2026/01/07/business/character-ai-google-settle-teen-suicide-lawsuit)

[^19]: The Human Line Project tracks cases of "AI psychosis." Nearly 300 documented incidents and at least 14 linked deaths as of early 2026. [humanlineproject.org](https://www.humanlineproject.org/)

[^20]: "Chegg slashes 45% of workforce, blames 'new realities of AI,'" CNBC, October 27, 2025. [cnbc.com](https://www.cnbc.com/2025/10/27/chegg-slashes-45percent-of-workforce-blames-new-realities-of-ai.html)

[^21]: "Chegg's CEO once said he's the 'poster child' for AI shock. Now, the company is slashing 45% of staff," Fortune, October 28, 2025. [fortune.com](https://fortune.com/2025/10/28/chegg-layoffs-ceo-dan-rosensweig-poster-child-ai-slashing-staff-shares/)

[^22]: Duolingo disclosed contractor cuts in January 2025. CEO Luis von Ahn stated AI could handle the translation and content work. [cnbc.com](https://www.cnbc.com/2025/01/08/duolingo-ai-replace-contract-workers.html)

[^23]: Stability AI: founder Emad Mostaque resigned March 2024 amid financial difficulties. Inflection AI: absorbed by Microsoft March 2024 after consumer product Pi failed. Jasper AI: multiple pivots through 2024-2025 as ChatGPT commoditised its market. [reuters.com](https://www.reuters.com/)

[^24]: "How Teens Use and View AI," Pew Research Center, February 24, 2026. Survey of 1,458 U.S. teens conducted September 25 – October 9, 2025. [pewresearch.org](https://www.pewresearch.org/internet/2026/02/24/how-teens-use-and-view-ai/)

[^25]: "What NYC's new AI school rules say, and what still remains unclear," Chalkbeat, March 24, 2026. [chalkbeat.org](https://www.chalkbeat.org/newyork/2026/03/24/preliminary-ai-policy-nyc-schools/)

[^26]: "Fed Regulation of AI Is Virtually Nonexistent. Is This a Problem for Schools?" EdWeek, January 2026. [edweek.org](https://www.edweek.org/technology/fed-regulation-of-ai-is-virtually-nonexistent-is-this-a-problem-for-schools/2026/01)

---

*Sage.Education is an open-source, self-hosted AI platform for education. It does not detect AI use. It does not replace teachers. It makes the learning process visible – to the student, to the educator, and to anyone evaluating whether a conversation with a machine produced understanding or just output. [sage.education](https://sage.education)*

*The views expressed are those of the editorial board. Sage.is AI-UI is a product of Startr LLC. The author has no financial relationship with OpenAI, Anthropic, Google, Character.AI, Khan Academy, Chegg, Alpha School, Turnitin, GPTZero, or the Human Line Project. Full disclosure and transparency is a feature, not a bug.*
